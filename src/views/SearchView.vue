...
const fetchSearchResults = async (query) => {
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/search?query=${encodeURIComponent(query)}&limit=40`,
    )
    if (response.ok) {
    ...
    <Container>
      ...
      <div class="flex flex-col gap-[24px]">
        ...
        <div class="flex items-center gap-[8px] body-m">
          <router-link
            to="/"
            class="text-text-light opacity-60 hover:opacity-100 transition-opacity"
            >Главная</router-link
          >
        ...
        <h1 class="text-4xl font-bold text-text-light">
          ...
          <span
            v-if="!isLoading && totalFound > 0"
            class="ml-4 text-xl text-text-light/40 font-normal"
          >
      ...
      <div class="mt-[64px] flex-grow">
        ...
        <!-- Results Grid -->
        <div
          v-else-if="allMovies.length > 0"
          class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-[32px]"
        >
          <MovieCard
        ...
        <div
          v-else-if="!isLoading"
          class="flex flex-col items-center justify-center py-[100px] text-center gap-[24px] bg-white/5 rounded-[32px] border border-white/5"
        >
          <div class="p-8 bg-white/5 rounded-full">
            <svg
              class="w-16 h-16 text-text-light/20"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
          ...
          <BaseButton variant="primary" size="l" class="px-8" @click="$router.push('/catalog')"
            >Перейти в каталог</BaseButton
          >
        </div>

        <!-- Load More Button -->
        <div
          v-if="!isLoading && displayCount < allMovies.length"
          class="flex justify-center mt-[64px]"
        >
          <BaseButton variant="secondary" size="l" @click="loadMore"> Показать еще </BaseButton>
        </div>
      </div>
    </Container>
  </main>
